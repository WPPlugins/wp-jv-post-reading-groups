...
/************************************************************************************************************/ 
/* Adds a Reading Groups metabox to Edit Post and Edit Page screen */
/************************************************************************************************************/
	...
	if(is_user_logged_in()) {		
		...
		if (is_archive()) {	
			...
			if (is_category()) {
				...
				$all_posts_in_category = get_posts(array('category_name' => $category,'post_status' =>array('private','publish'),'posts_per_page'=>-1));
...
//Start here: wp-includes\widgets\class-wp-widget-tag-cloud.php
// and here: https://developer.wordpress.org/reference/hooks/wp_tag_cloud/
...
function wp_jv_prg_remove_private_pages_from_menu_items( $items, $menu, $args ) {
	foreach ( $items as $key => $item ) {
		...
		if ($item->object=='category') {
			if(!is_user_logged_in() ||	!wp_jv_prg_user_can_see_a_category (get_current_user_id(),$item->object_id)) {
					unset( $items[$key] );
				}
		} 
	}
